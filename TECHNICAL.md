...
* **No snapshot pinning.** Correctness instead comes from gating: live streaming (`start_streaming()`) only waits on the dump phase (Postgres→SQLite, cheap, no embedding) to finish, not the full drain — draining runs concurrently with live streaming.
