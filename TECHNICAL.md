...
* **A `LlamaProvider`'s LoRA adapter (`lora_path`) is fixed for that process's whole lifetime — there's no runtime hot-swap between tasks.** `llama_set_adapters_lora()` is called once in `init()`; nothing in the request path calls it again per `embed()`/`embed_batch()` call. For an instruction-tuned model with asymmetric query/passage adapters (e.g. `jina-embeddings-v3`), this means one running walkrie instance can only embed one "kind" of text well — a pipeline indexing rows and a tool embedding search queries need two separate `[embedding]` configs (same `model_path`, different `lora_path`), not one config doing both. See the `lora_path`/`lora_scale` note under Configuration Syntax and `demo/README_ecommerce.md` for the concrete two-config pattern.
...
lora_path  = ""         # optional LoRA adapter GGUF — see note below; "" (default) = no adapter
lora_scale = 1.0        # ignored if lora_path is empty
dimensions = 1024
n_threads  = 4
n_ctx      = 512
n_gpu_layers = 0        # 0 (default, CPU-only) | N layers to offload to GPU — see note below
...
**Note on `lora_path`/`lora_scale`** (llama provider only): loads a LoRA adapter GGUF via `llama_adapter_lora_init()` and activates it via `llama_set_adapters_lora()` once, in `LlamaProvider::init()` — it stays active for that provider instance's whole lifetime, there's no per-request switching. This matters for instruction-tuned multilingual models (e.g. `jina-embeddings-v3`) whose GGUF conversion splits each task into its own adapter file (`retrieval.query`, `retrieval.passage`, etc. — see `gaianet/jina-embeddings-v3-GGUF`): a pipeline embedding rows being indexed needs a different adapter than a tool embedding search queries, so each needs its own `[embedding]` config pointing at the same `model_path` but a different `lora_path`. If the adapter's GGUF metadata carries an `adapter.lora.prompt_prefix` key (also true of `jina-embeddings-v3`'s adapters — the model expects that exact instruction string prepended to input text), `LlamaProvider` reads it automatically and prepends it in `tokenize_batch()`; nothing to configure for that part. See [`config_samples/config_sample_llama_lora.toml`](./config_samples/config_sample_llama_lora.toml) for a full example, and [`demo/README_ecommerce.md`](./demo/README_ecommerce.md) for a worked multilingual (Japanese/Turkish) walkthrough covering the two-config split.
